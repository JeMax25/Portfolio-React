        ...
        <div className="flex">
            <nav className={`nav-bar-sm flex flex-col md:flex-row items-center justify-center gap-2 absolute md:static top-0 bg-slate-800 dark:bg-slate-100 md:bg-transparent dark:md:bg-transparent w-1/2 h-screen md:w-auto md:h-auto text-center z-20 ${ menuActive ? 'left-1/2' : 'left-full'}`}>
                <ul className="h-1/2 flex flex-col md:flex-row gap-5 md:gap-6 lg:gap-12 text-lg md:text-xl">
                    ...
                    <li><a href="#" onClick={handleMenu} className="cursor-pointer">Inicio</a></li>
                    <li onClick={() => {scrollToElement('projects'); handleMenu()}} className="cursor-pointer">Proyectos</li>
                    <li onClick={() => {scrollToElement('About'); handleMenu() }} className="cursor-pointer">Información</li>
                </ul>
                <ul className="flex justify-center gap-2 lg:ml-7 lg:mr-7 md:ml-4 md:mr-4 ml-3 mr-3 w-full ">
                    <li className="bg-zinc-950 border-slate-900 dark:bg-slate-50 dark:border-slate-50 h-8 w-8 text-center border-solid border-2 rounded-full"><a href="https://acortar.link/wQpMsm" target="_blank"><img src={`${themeIcons ? './assets/svg-white/linkedin.svg' : './assets/svg/linkedin.svg'}`} alt="linkedin"/></a></li>
                    <li className="bg-zinc-950 border-slate-900 dark:bg-slate-50 dark:border-slate-50 h-8 w-8 text-center border-solid border-2 rounded-full"><a href="https://github.com/JeMax25" target="_blank"><img src={`${themeIcons ? './assets/svg-white/github.svg' : './assets/svg/github.svg'}`} alt="git" className="w-full"/></a></li>
                </ul>   
            </nav>
            <div onClick={handleMenu} className="md:hidden w-8 flex justify-center cursor-pointer">
